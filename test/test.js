...
describe("Multigrain", function () {
  ...
  it("should convert explicit CSON to JSON", function () {
    assert.equal(
      multigrain.json('key:"value"', "cson"),
      '{\n\t"key": "value"\n}'
    );
  });
  it("should convert explicit YAML to JSON", function () {
    assert.equal(
      multigrain.json("key: value", "yaml"),
      '{\n\t"key": "value"\n}'
    );
  });
  it("should convert explicit PLIST to JSON", function () {
    assert.equal(
      multigrain.json(
        '<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd"><plist version="1.0"><dict><key>key</key><string>value</string></dict></plist>',
        "plist"
      ),
      '{\n\t"key": "value"\n}'
    );
  ...
  it("should convert explicit TOML to JSON", function () {
    assert.equal(
      multigrain.json('key = "value"', "toml"),
      '{\n\t"key": "value"\n}'
    );
  });
  ...
  it("should convert explicit JSON to PLIST", function () {
    assert.equal(
      multigrain.plist('{"key":"value"}', "json"),
  ...
  it("should parse CSON into a JavaScript object", function () {
    assert.deepEqual(multigrain.parse('{"key":"value"}', "json"), {
      key: "value",
    });
  });
  it("should parse YAML into a JavaScript object", function () {
    assert.deepEqual(multigrain.parse('{"key":"value"}', "yaml"), {
      key: "value",
    });
  });
  ...
  it("should parse TOML into a JavaScript object", function () {
    assert.deepEqual(multigrain.parse('key = "value"', "toml"), {
      key: "value",
    });
  });
  ...
  it("should respect CSON builder options", function () {
    assert.equal(
      multigrain.cson('{"a":{"b":"c"}}', "json", null, { indent: "\t\t" }),
      "a:\n\t\tb: 'c'"
    );
  });
